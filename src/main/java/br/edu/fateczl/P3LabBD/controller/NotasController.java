...
import br.edu.fateczl.P3LabBD.model.dto.AvaliacaoDTO;
...
import br.edu.fateczl.P3LabBD.persistence.NotasRelatorioRepository;
import br.edu.fateczl.P3LabBD.persistence.NotasRepository;
...
public class NotasController implements INotaController{
	...
	@Override
	@PutMapping("/notas")
	public ResponseEntity<String> atualizaNotas(
			@Valid @RequestBody Notas notas) {
		String saida = nRep.spAtualizaNotas(notas.getNota(),
				notas.getAluno().getRa(), 
				notas.getDisciplina().getCodigo(), 
				notas.getAvaliacao().getCodigo());
	...
	public List<NotasRelatorioDTO> listaNotasAlunoDisc(@PathVariable(value = "ra") int ra, @PathVariable(value = "codigo") int codigo) {
	...
	@Override
	@PostMapping("/notas")
	public ResponseEntity<String> insereNotas(
		@Valid @RequestBody Notas notas) {
			String saida = nRep.spInsereNotas(
					notas.getAluno().getRa(), 
					notas.getDisciplina().getCodigo(), 
					notas.getNota());
			return ResponseEntity.ok().body(saida);
