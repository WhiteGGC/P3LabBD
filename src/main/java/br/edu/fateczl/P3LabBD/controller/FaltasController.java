...
import br.edu.fateczl.P3LabBD.persistence.FaltasRelatorioRepository;
import br.edu.fateczl.P3LabBD.persistence.FaltasRepository;
...
public class FaltasController implements IFaltaController{
	...
	@Override
	@PutMapping("/faltas")
	public ResponseEntity<String> atualizaFaltas(
			@Valid @RequestBody Faltas faltas) {
		String saida = fRep.spAtualizaFaltas(faltas.getPresenca(),
				faltas.getAluno().getRa(), 
				faltas.getDisciplina().getCodigo(), 
				faltas.getData());
	...
	public List<FaltasRelatorioDTO> listaFaltasAlunoDisc(@PathVariable(value = "ra") int ra, @PathVariable(value = "codigo") int codigo) {
